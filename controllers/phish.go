...
func (ps *PhishingServer) registerRoutes() {
	...
	router.HandleFunc("/{path:.*}/resource/image/pixel.png", ps.TrackHandler)
	router.HandleFunc("/{path:.*}/api/v1/status", ps.ReportHandler)
	router.HandleFunc("/api/v1/status", ps.ReportHandler)
	// Conditionally enable QR code endpoint based on config
	if ps.config.EnableQRCode {
		router.HandleFunc("/qrcode", ps.QRHandler)
		log.Info("QR code endpoint enabled at /qrcode")
	}
...
func (ps *PhishingServer) PhishHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Header "X-Server" removed for stealth
...
// QRHandler generates a QR code for the provided text
...
// fakeNotFound writes a standard Nginx 404 response to the ResponseWriter
func fakeNotFound(w http.ResponseWriter, r *http.Request) {
